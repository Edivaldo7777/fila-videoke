...
export default function Dashboard() {
  ...
  const roomUrl =
    roomCode !== "" ? `http://localhost:3000/room/${roomCode}` : "";
  ...
  return (
    <main className="min-h-screen bg-slate-100 p-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-4xl font-bold">🎤 Painel do Operador</h1>
          {currentUser && (
            <div className="mt-2 text-sm text-slate-600">
              ...
              {currentUser.role !== "admin" && (
                <div>
                  Salas utilizadas: {rooms.length} / {currentUser.max_rooms}
                </div>
              )}
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div className="mb-4">
          <label className="block font-bold mb-2">Nome da Sala</label>
          <input
            type="text"
            value={roomName}
        ...
        <div className="mb-4">
          <label className="block font-bold mb-2">Modo do Evento</label>
          <select
            value={eventMode}
        ...
        {/* Novo campo para escolher o Tipo de Votação */}
        <div className="mb-4">
          <label className="block font-bold mb-2">Tipo de Votação</label>
          <select
            value={votingMode}
            onChange={(e) => setVotingMode(e.target.value)}
            className="border rounded p-2 w-full bg-white"
          >
            <option value="stars">⭐ Estrelas (Tradicional de 1 a 5)</option>
            <option value="thumbs">👍👎 Aprovação (Bom / Ruim com conversão 0 a 100)</option>
          </select>
        </div>
      ...
      {roomCode && (
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <div className="mt-4 inline-block border p-4 rounded">
            <QRCode value={roomUrl} size={220} />
          </div>

          <p className="mt-4 text-sm break-all">{roomUrl}</p>
        </div>
      )}

      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-2xl font-bold mb-4">🎵 Salas Existentes</h2>
        ...
        {rooms.length === 0 ? (
          ...
          <div className="space-y-4">
            {rooms.map((room) => (
              <div key={room.room_code} className="border rounded p-4">
                <h3 className="text-xl font-bold">{room.room_name}</h3>
                <p className="mb-3">Código: {room.room_code}</p>
                <p className="mb-3 font-bold">
                  {room.status === "encerrada" ? (
                    <span className="text-red-600">🔴 ENCERRADA</span>
                  ) : (
                ...
                {room.status === "encerrada" && (
                  <button
                    onClick={() => startNewEvent(room.room_code)}
                ...
                <div className="flex gap-2 flex-wrap">
                  <button
                    onClick={() =>
                      window.open(`/room/${room.room_code}`, "_blank")
                    }
                  ...
                  <button
                    onClick={() =>
                      window.open(`/operator/${room.room_code}`, "_blank")
                    }
                  ...
                  <button
                    onClick={() =>
                      window.open(`/tv/${room.room_code}`, "_blank")
                    }
