...
export default function OperatorPage({
  ...
  async function nextSinger() {
    ...
    const { data: room, error: roomError } = await supabase
      .from("rooms")
      .select("room_code, current_event_id")
      .eq("room_code", roomCode)
      .single();
    ...
    const { data: performance, error: performanceError } = await supabase
      ...
      .insert({
        room_code: roomCode,
        event_id: room.current_event_id,
    ...
    if (performanceError || !performance) {
      console.error("Erro ao registrar apresentação:", performanceError);
      alert(performanceError?.message || "Erro ao registrar apresentação.");
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-8">
      <h1 className="text-5xl font-black mb-2">🎤 {roomName || "VIDEOKÊ"}</h1>
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-slate-800 rounded-xl p-6 text-center">
          <div className="text-4xl">🎤</div>
          <div className="font-bold text-lg">{currentSinger?.singer_name || "Aguardando"}</div>
      ...
      <div className="bg-yellow-400 text-black rounded-xl p-6 mb-6">
        ...
        <div className="text-4xl font-black">{currentSinger?.singer_name || "Aguardando"}</div>
        <div className="text-xl mt-2">🎵 {currentSinger?.song_name || "Nenhuma música"}</div>
        ...
        {/* Exibe a mensagem personalizada se o modo for thumbs */}
        {votingMode === "thumbs" && currentVotes > 0 && (
          <div className="mt-4 bg-black text-white p-3 rounded-lg text-center font-bold text-lg">
      ...
      <div className="bg-slate-800 rounded-xl p-6 mb-6">
        ...
        <input
          ...
          className="w-full p-3 rounded bg-slate-700 mb-3"
        />
        <button onClick={addSinger} className="bg-blue-600 hover:bg-blue-700 px-5 py-3 rounded">
          Adicionar à Fila
        </button>
      </div>

      <div className="flex flex-wrap gap-3 mb-6">
        <button onClick={nextSinger} className="bg-green-600 hover:bg-green-700 px-5 py-3 rounded">
          Próximo Cantor
        </button>

        <button onClick={clearQueue} className="bg-red-600 hover:bg-red-700 px-5 py-3 rounded">
          Limpar Fila
        </button>
        ...
        {!eventEnded && (
          <button
            ...
            className={`px-5 py-3 rounded ${
              endingEvent ? "bg-slate-600 cursor-not-allowed" : "bg-purple-700 hover:bg-purple-800"
            }`}
          >
            {endingEvent ? "⏳ Encerrando..." : "🔚 Encerrar Evento"}
          </button>
        )}
      </div>
      ...
      {eventEnded && (
        <div className="bg-purple-900 border border-purple-500 rounded-xl p-6 mb-6 text-center">
          <h2 className="text-2xl font-black mb-2">🏆 Evento Encerrado</h2>
          <p className="text-purple-100">A TV está exibindo a premiação da noite.</p>
          <button
            onClick={() => {
      ...
      <div className="space-y-3">
        {queue.length === 0 ? (
          <div className="bg-slate-800 rounded-xl p-6">Nenhum participante na fila.</div>
        ) : (
          queue.map((item, index) => (
            <div key={item.id} className="bg-slate-800 rounded-xl p-4 flex justify-between items-center">
              <div>
              ...
              <button onClick={() => removeItem(item.id)} className="bg-red-500 px-3 py-2 rounded">
                Remover
              </button>
            </div>
          ))
