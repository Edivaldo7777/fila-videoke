...
export default function OperatorPage({
  ...
  // NOVO USE EFFECT: Com Supabase Realtime
  useEffect(() => {
    if (!roomCode) return;
    ...
    async function validateAccess() {
      ...
      if (user.role === "admin" || room.owner_id === user.id) {
        ...
        // Inscrevendo no Supabase Realtime
        channel = supabase
          ...
          .on(
            "postgres_changes",
            { event: "*", schema: "public", table: "performances", filter: `room_code=eq.${roomCode}` },
            () => loadData()
          )
          .subscribe();
      }
  ...
  async function loadVotes(singerToken: string) {
    ...
    const total = votes.reduce((sum, vote) => sum + Number(vote.score), 0);
  ...
  async function nextSinger() {
    ...
    await supabase.from("current_singer").upsert({
      room_code: roomCode,
    ...
    if (performanceError) {
      console.error("Erro performances:", performanceError);
      alert(`Erro ao registrar performance: ${performanceError.message}`);
    }

    const { data: profile } = await supabase
      .from("singer_profile")
      .select("*")
      .eq("singer_token", singer.singer_token)
      .single();

    let nextSong = "Escolherá na hora de cantar";

    if (profile?.next_song && profile.next_song.trim() !== "") {
    ...
    await supabase
      .from("singer_profile")
      .update({ next_song: "Escolherá na hora de cantar" })
      .eq("singer_token", singer.singer_token);

    await supabase.from("queue").delete().eq("id", singer.id);
  ...
  // NOVO END EVENT: Com filtro de sala e código limpo
  async function endEvent() {
    ...
    try {
      const { data: votes } = await supabase
        .from("singer_votes")
        .select("*")
        .eq("room_code", roomCode); // Correção crítica de segurança
      ...
      if (votes && votes.length > 0) {
        ...
        votes.forEach((vote: any) => {
          ...
          rankingMap[vote.singer_token].total += Number(vote.score);
          rankingMap[vote.singer_token].count += 1;
        });

        if (performances) {
          performances.forEach((performance: any) => {
            if (rankingMap[performance.singer_token]) {
              rankingMap[performance.singer_token].presentations += 1;
            }
          });
        }

        const ranking = await Promise.all(
          Object.values(rankingMap).map(async (item: any) => {
            const { data: singer } = await supabase
              .from("singer_profile")
              .select("*")
              .eq("singer_token", item.singer_token)
              .single();

            return {
              singer_name: singer?.singer_name || "Desconhecido",
        ...
        const champions = ranking
          .filter((item) => item.presentations >= 3)
        ...
        const champion = champions.length > 0 ? champions[0] : null;
        const revelation = revelations.length > 0 ? revelations[0] : null;
        ...
        if (performances && performances.length > 0) {
          const performanceRanking = performances.map((performance: any) => {
            const singerData = ranking.find(
              (item: any) => item.singer_name === performance.singer_name
            );

            return {
              singer_name: performance.singer_name,
              song_name: performance.song_name,
              average: singerData?.average || 0,
            };
          });

          performanceRanking.sort((a: any, b: any) => b.average - a.average);

          if (performanceRanking[0]) {
            bestSongName = performanceRanking[0].song_name;
            bestSongSinger = performanceRanking[0].singer_name;
            bestSongAverage = performanceRanking[0].average;
          }
        }
        ...
        await supabase.from("hall_of_fame").insert({
          ...
          best_song_name: bestSongName,
          best_song_singer: bestSongSinger,
          best_song_average: bestSongAverage,
          total_presentations: champion?.presentations || 0,
        });

        let message = "🏆 PREMIAÇÃO DA NOITE\n\n";
        ...
        if (bestSongName) {
          message +=
            ...
            `${bestSongName}\n` +
            `${bestSongSinger}\n` +
            `⭐ ${Number(bestSongAverage).toFixed(2)}`;
        }
        ...
        // Atualização única e limpa do status da sala
        ...
        if (roomStatusError) {
          throw new Error(`Erro ao atualizar status da sala: ${roomStatusError.message}`);
        }
      }
      ...
      console.error(error);
      alert(`Ocorreu um erro ao encerrar o evento: ${error.message}`);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-8">
      ...
      <p className="text-slate-400 mb-8">Código da Sala: {roomCode}</p>

      <div className="grid md:grid-cols-3 gap-4 mb-6">
        <div className="bg-slate-800 rounded-xl p-6 text-center">
          <div className="text-4xl">👥</div>
          <div className="text-3xl font-bold">{queue.length}</div>
          <p className="text-slate-400">Na fila</p>
        </div>
        ...
        <div className="bg-slate-800 rounded-xl p-6 text-center">
          ...
          <p className="text-slate-400">Cantando Agora</p>
        </div>
      </div>

      <div className="bg-yellow-400 text-black rounded-xl p-6 mb-6">
        <h2 className="text-2xl font-black mb-2">🎤 AGORA CANTANDO</h2>
        ...
        <div className="grid grid-cols-2 gap-4 mt-5">
          <div className="bg-white rounded p-4 text-center">
            <div className="text-sm">Nota Média</div>
            <div className="text-3xl font-black">
              ⭐ {currentScore.toFixed(1)}
            </div>
          </div>
          <div className="bg-white rounded p-4 text-center">
            <div className="text-sm">Total de Votos</div>
      ...
      <div className="bg-slate-800 rounded-xl p-6 mb-6">
        <h2 className="text-2xl font-bold mb-4">➕ Adicionar Cantor</h2>
        <input
          type="text"
          value={newSinger}
          ...
          queue.map((item, index) => (
            ...
            >
              <div>
                ...
                <div className="text-slate-400">🎵 {item.song_name}</div>
              </div>
              <button
